...
- current public path:
  - `<SHARED_ROOT>/scripts`
- new canonical path:
  - `<SHARED_ROOT>/projects/ohos-helper`
- final parent entry:
  - `<SHARED_ROOT>/scripts -> <SHARED_ROOT>/projects/ohos-helper`
...
- multiple tests hardcode `<SHARED_ROOT>/scripts/...`
...
- changing download/cache roots such as `<SHARED_ROOT>/xts_tests`
...
- `<SHARED_ROOT>/scripts/ohos.sh`
- `<SHARED_ROOT>/scripts/ohos_device.sh`
...
- `ohos.sh requires bash. Run it with: bash <SHARED_ROOT>/scripts/ohos.sh ...`
...
```bash
ohos admin relocate --target-root <SHARED_ROOT>/projects/ohos-helper --legacy-link <SHARED_ROOT>/scripts --dry-run
```
