...
from test_helpers import get_paths, get_ssh
...
case "${1:-}" in
  -I)
    echo "{SSH['bridge_host']} 127.0.0.1"
    ;;
  -f)
    echo "{SSH['bridge_host']}.example.net"
    ;;
  *)
    echo "{SSH['bridge_host']}"
    ;;
esac
""",
        )
...
""",
    ...
    def test_npmrc_mirror_profile_stays_default_for_sync(self):
        env = self.env.copy()
        env["NPM_REGISTRY"] = "http://registry.example.com/repository/huaweicloud"
        ...
        self.assertIn("registry=http://registry.example.com/repository/huaweicloud", result.stdout)
    ...
    def test_device_bridge_package_windows_auto_detects_server_host_and_user(self):
        ...
        self.assertEqual(argv[argv.index("--server-host") + 1], SSH["bridge_host"])
        self.assertEqual(argv[argv.index("--server-user") + 1], "deviceuser")
        self.assertIn(f"Auto-detected Linux test server address: {SSH['bridge_host']}", result.stdout)
    ...
    def test_download_tests_uses_shared_common_roots_by_default(self):
        ...
        self.assertEqual(argv[argv.index("--daily-cache-root") + 1], PATHS["xts_tests"])
        self.assertIn("--sdk-cache-root", argv)
        self.assertEqual(argv[argv.index("--sdk-cache-root") + 1], PATHS["sdk"])
        self.assertIn("--firmware-cache-root", argv)
        self.assertEqual(argv[argv.index("--firmware-cache-root") + 1], PATHS["firmwares"])
