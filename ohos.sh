...
OHOS_BUILD_WORKAROUNDS_LIB="${OHOS_BUILD_WORKAROUNDS_LIB:-${SCRIPT_DIR}/ohos_build_workarounds.sh}"
...
OHOS_PACK_TOOL="${OHOS_PACK_TOOL:-${SCRIPT_DIR}/ohos_pack.py}"
OHOS_SIGN_TOOL="${OHOS_SIGN_TOOL:-${SCRIPT_DIR}/ohos_sign.sh}"
...
if [ -f "$OHOS_BUILD_WORKAROUNDS_LIB" ]; then
    # shellcheck disable=SC1090
    source "$OHOS_BUILD_WORKAROUNDS_LIB"
fi

NPM_REGISTRY="${NPM_REGISTRY:-https://repo.huaweicloud.com/repository/npm/}"
OHOS_NPM_REGISTRY="${OHOS_NPM_REGISTRY:-https://repo.harmonyos.com/npm/}"
OHPM_REGISTRY="${OHPM_REGISTRY:-https://repo.harmonyos.com/ohpm/}"
PYPI_URL="${PYPI_URL:-https://repo.huaweicloud.com/repository/pypi/simple/}"
TRUSTED_HOST="${TRUSTED_HOST:-}"
KOALA_NPM_REGISTRY="${KOALA_NPM_REGISTRY:-}"
...
OHOS_NO_PROXY="${OHOS_NO_PROXY:-localhost,127.0.0.1}"
...
CCACHE_UMASK="${CCACHE_UMASK:-000}"
CCACHE_NOSUPPORT="${CCACHE_NOSUPPORT:-1}"
...
SHARED_PREBUILTS_DIR="${SHARED_PREBUILTS_DIR:-}"
PREBUILTS_SYMLINK_NAME="${PREBUILTS_SYMLINK_NAME:-openharmony_prebuilts}"

# ---- Internal constants (not user-configurable) --------------------------------
OHOS_SIGNAL_RETRY_DELAY="${OHOS_SIGNAL_RETRY_DELAY:-0.1}"
OHOS_SIGNAL_RETRY_COUNT="${OHOS_SIGNAL_RETRY_COUNT:-5}"
OHOS_PROGRESS_UPDATE_INTERVAL="${OHOS_PROGRESS_UPDATE_INTERVAL:-1}"
OHOS_NPMRC_BACKUP_SUFFIX=".ohosbak"
OHOS_STALE_LOCK_MAX_AGE_SEC="${OHOS_STALE_LOCK_MAX_AGE_SEC:-300}"
...
# Run command with LFS environment variables unset and GIT_LFS_STORAGE set to .git/lfs.
# Usage: _lfs_env git lfs fetch --all
_lfs_env() {
    env -u LocalMediaDir -u LocalReferenceDirs -u TempDir -u LfsStorageDir \
        GIT_LFS_STORAGE=.git/lfs "$@"
}

# String form of _lfs_env for embedding in repo forall -c commands.
_lfs_env_prefix() {
...
ohos_forward_signal() {
    ...
    if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
        ...
        for _attempt in $(seq 1 "$OHOS_SIGNAL_RETRY_COUNT"); do
            if ! kill -0 "$pid" 2>/dev/null; then
                OHOS_ACTIVE_CHILD_PID=""
                OHOS_ACTIVE_CHILD_PGID=""
                return 0
            fi
            sleep "$OHOS_SIGNAL_RETRY_DELAY"
        done
        if [ -n "$pgid" ] && [[ "$pgid" =~ ^[0-9]+$ ]]; then
...
install_gitee_util_user_deps() {
    ensure_python3
    # --force-reinstall overrides broken system packages (e.g. old
    # prompt_toolkit 1.0.14 in /usr/local/ incompatible with Python 3.10)
    # with working versions in ~/.local/lib.
...
ensure_gitee_util_runtime() {
    # Some Python builds (e.g. Ubuntu 22.04 system python) disable user
    # site-packages. If ~/.local/lib exists but is not in sys.path,
    # add it via PYTHONPATH so pip --user installs are found.
    local _user_site
    _user_site="$(python3 -m site --user-site 2>/dev/null || true)"
    if [ -n "$_user_site" ] && [ -d "$_user_site" ]; then
        if ! python3 -c "import sys; exit(0 if '$_user_site' in sys.path else 1)" 2>/dev/null; then
            GITEE_UTIL_PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$_user_site"
        fi
    fi

    if { [ -z "${GITEE_UTIL_PYTHONPATH:-}" ] && python_has_modules python3 requests tqdm prompt_toolkit bs4 dateutil; } ||
       { [ -n "${GITEE_UTIL_PYTHONPATH:-}" ] && PYTHONPATH="$GITEE_UTIL_PYTHONPATH" python_has_modules python3 requests tqdm prompt_toolkit bs4 dateutil; }; then
    ...
    if confirm_default_yes "Install missing gitee_util Python deps into ~/.local now? [Y/n] "; then
        ...
        err "Diagnostic: sys.path=$(python3 -c 'import sys; print(sys.path)')"
        err "User site: $_user_site"
        err "GITEE_UTIL_PYTHONPATH=${GITEE_UTIL_PYTHONPATH:-<unset>}"
        exit 1
    fi
...
fix_lfs_storage() {
    ...
    # Use globstar to match project configs at any directory depth under
    # .repo/projects/ — repo tool places them at 1–4+ levels depending
    # on manifest path (e.g. build.git vs foundation/arkui/ace_engine.git).
    shopt -s globstar
...
force_clean_repo_path() {
    ...
    if ! git -C "$repo_path" -c lfs.fetchexclude="*" reset --hard HEAD 2>/dev/null; then
        # Only remove if the bare repo in .repo/projects/ still exists —
        # confirms this is a metadata mismatch, not e.g. a full disk.
        local _bare_path=".repo/projects/${repo_path}.git"
        if [ -d "$_bare_path" ]; then
            warn "git reset failed for $repo_path — removing for re-clone."
            rm -rf "$repo_path"
            return 0
        fi
        err "git reset failed for $repo_path and .repo/projects bare repo is missing — cannot auto-recover."
        return 1
    fi
...
configure_ccache() {
    ...
    export CCACHE_UMASK
    export CCACHE_NOSUPPORT
    # OH build system (build_args_resolver.py:resolve_ccache) computes CCACHE_DIR
    # as $CCACHE_BASE/$CCACHE_LOCAL_DIR, defaulting to $HOME/.ccache.
    # It overwrites CCACHE_DIR on line 388. Set CCACHE_BASE so the build system
    # derives the same path ohos.sh configured.
    export CCACHE_BASE="${CCACHE_BASE:-${CCACHE_DIR%/*}}"
    export CCACHE_LOCAL_DIR="${CCACHE_LOCAL_DIR:-${CCACHE_DIR##*/}}"
...
sync_stage_repo() {
    ...
    while true; do
            ...
            if [ ${#_lfs_failed_paths[@]} -gt 0 ]; then
                ...
                for _lfs_repo in "${_lfs_failed_paths[@]}"; do
                    if [ -d "$_lfs_repo" ] && git -C "$_lfs_repo" rev-parse --git-dir >/dev/null 2>&1; then
                        ...
                        (cd "$_lfs_repo" && _lfs_env git lfs fetch --all 2>&1 && _lfs_env git lfs checkout 2>&1) || true
                    fi
                done
        ...
        if [ "$SYNC_FORCE_ENABLED" != "true" ]; then
            ...
            # Detect "info is different" / "cannot overwrite a local work tree"
            # errors that require --force-sync.  Auto-recover by force-cleaning
            # only the affected project paths and retrying.
            if grep -q 'info is different.*\.git vs\|config is different.*\.git vs\|--force-sync not enabled\|cannot overwrite a local work tree' "$LAST_STEP_LOG" 2>/dev/null; then
                if [ ${#_hint_paths[@]} -gt 0 ]; then
                    local _dirty_paths=()
                    local _clean_paths=()
                    local _check_p
                    for _check_p in "${_hint_paths[@]}"; do
                        local _has_real_changes=false
                        if [ -d "$_check_p" ]; then
                            local _rc1=0 _rc2=0
                            git -C "$_check_p" diff --quiet HEAD 2>/dev/null || _rc1=$?
                            git -C "$_check_p" diff --quiet --cached 2>/dev/null || _rc2=$?
                            # exit 1 = actual changes; exit >1 = git error (broken metadata)
                            # Broken metadata → cannot reliably detect changes → safe to force-recover
                            if [ "$_rc1" -eq 1 ] || [ "$_rc2" -eq 1 ]; then
                                _has_real_changes=true
                            fi
                        fi
                        if "$_has_real_changes"; then
                            _dirty_paths+=("$_check_p")
                        else
                            _clean_paths+=("$_check_p")
                        fi
                    done
                    if [ ${#_dirty_paths[@]} -gt 0 ]; then
                        warn "Cannot auto-recover: ${#_dirty_paths[@]} project(s) have uncommitted changes:"
                        printf '%s\n' "${_dirty_paths[@]}" | sed 's/^/[ohos]   /' >&2
                        err "To force-sync (will discard changes):"
                        for _dp in "${_dirty_paths[@]}"; do
                            err "  ohos sync -f $_dp"
                        done
                        if [ ${#_clean_paths[@]} -eq 0 ]; then
                            return "$rc"
                        fi
                        warn "Auto-recovering ${#_clean_paths[@]} clean project(s):"
                        printf '%s\n' "${_clean_paths[@]}" | sed 's/^/[ohos]   /' >&2
                    else
                        warn "Detected git metadata mismatch for ${#_hint_paths[@]} project(s). Auto-recovering with force-sync:"
                        printf '%s\n' "${_hint_paths[@]}" | sed 's/^/[ohos]   /' >&2
                        _clean_paths=("${_hint_paths[@]}")
                    fi
                    force_clean_repo_paths "${_clean_paths[@]}"
                    target_paths=("${_clean_paths[@]}")
                    jobs=1
                    fail_fast=true
                    attempt=$((attempt + 1))
                    if [ "$attempt" -gt "$SYNC_FORCE_MAX_RETRIES" ]; then
                        err "Force-sync auto-retry limit reached (${SYNC_FORCE_MAX_RETRIES})."
                        return "$rc"
                    fi
                    continue
                fi
...
sync_stage_lfs() {
    local rc
    local lfs_env_prefix
    lfs_env_prefix="$(_lfs_env_prefix)"
...
cmd_sync() {
    ...
    if [ ${#sync_projects[@]} -gt 0 ]; then
        ...
        clean_stale_git_locks
        local _sync_log
        _sync_log="$(mktemp /tmp/ohos_project_sync_XXXXXX.log)"
        local sync_rc=0
        repo sync -j "$REPO_SYNC_JOBS" --optimized-fetch --current-branch --retry-fetches=5 "${resolved_paths[@]}" 2>&1 | tee "$_sync_log" || sync_rc=$?
        if [ "$sync_rc" -ne 0 ]; then
            if grep -q 'info is different.*\.git vs\|config is different.*\.git vs\|--force-sync not enabled\|cannot overwrite a local work tree' "$_sync_log" 2>/dev/null; then
                local _has_dirty=false
                local _rp
                for _rp in "${resolved_paths[@]}"; do
                    local _rc1=0 _rc2=0
                    git -C "$_rp" diff --quiet HEAD 2>/dev/null || _rc1=$?
                    git -C "$_rp" diff --quiet --cached 2>/dev/null || _rc2=$?
                    if [ "$_rc1" -eq 1 ] || [ "$_rc2" -eq 1 ]; then
                        _has_dirty=true
                        break
                    fi
                done
                if [ "$_has_dirty" = "true" ]; then
                    warn "Cannot auto-recover: project has uncommitted changes."
                    err "To force-sync (will discard changes): ohos sync -f ${sync_projects[*]}"
                    return "$sync_rc"
                fi
                warn "Detected git metadata mismatch. Force-cleaning and retrying..."
                force_clean_repo_paths "${resolved_paths[@]}"
                repo sync -j1 --force-sync --optimized-fetch --current-branch "${resolved_paths[@]}"
                sync_rc=$?
                if [ "$sync_rc" -ne 0 ]; then
                    err "Force-sync retry also failed."
                    return "$sync_rc"
                fi
                info "Force-sync recovery succeeded."
            else
        ...
        for _sp in "${resolved_paths[@]}"; do
            if [ -d "$_sp" ]; then
                ...
                local _lfs_rc=0
                (cd "$_sp" && _lfs_env git lfs checkout 2>&1) || _lfs_rc=$?
                if [ "$_lfs_rc" -ne 0 ]; then
                    warn "LFS checkout failed for $_sp (exit $_lfs_rc), retrying..."
                    (cd "$_sp" && _lfs_env git lfs fetch --all 2>&1 && _lfs_env git lfs checkout 2>&1) || {
...
cmd_reset() {
    ...
    if [ ${#reset_projects[@]} -eq 1 ]; then
        ...
        clean_stale_git_locks
        local _reset_sync_log
        _reset_sync_log="$(mktemp /tmp/ohos_reset_sync_XXXXXX.log)"
        local sync_rc=0
        repo sync -j "$REPO_SYNC_JOBS" --optimized-fetch --current-branch "$project" 2>&1 | tee "$_reset_sync_log" || sync_rc=$?
        if [ "$sync_rc" -ne 0 ]; then
            if grep -q 'info is different.*\.git vs\|config is different.*\.git vs\|--force-sync not enabled\|cannot overwrite a local work tree' "$_reset_sync_log" 2>/dev/null; then
                warn "Git metadata mismatch detected. Retrying with --force-sync..."
                rm -rf "$project"
                repo sync -j1 --force-sync --optimized-fetch --current-branch "$project"
                sync_rc=$?
            fi
            if [ "$sync_rc" -ne 0 ]; then
                err "repo sync failed for $project (exit $sync_rc)"
                err "  retry with: ohos sync $project"
                rm -f "$_reset_sync_log"
                exit "$sync_rc"
            fi
        fi
        rm -f "$_reset_sync_log"
        ...
        if [ -d "$project" ]; then
            ...
            local _lfs_rc=0
            (cd "$project" && _lfs_env git lfs checkout 2>&1) || _lfs_rc=$?
            if [ "$_lfs_rc" -ne 0 ]; then
                warn "LFS checkout failed for $project (exit $_lfs_rc), retrying..."
                (cd "$project" && _lfs_env git lfs fetch --all 2>&1 && _lfs_env git lfs checkout 2>&1) || {
    ...
    run_step "[$step/$total] clean all sub-repos (git clean -fxd)" \
        repo forall -j "$RESET_JOBS" -c 'git -c filter.lfs.smudge= -c filter.lfs.process= -c filter.lfs.required=false clean -fxd'
    step=$((step + 1))

    run_step "[$step/$total] hard-reset all sub-repos (git reset --hard HEAD)" \
        repo forall -j "$RESET_JOBS" -c 'git -c filter.lfs.smudge= -c filter.lfs.process= -c filter.lfs.required=false reset --hard HEAD'
    step=$((step + 1))
...
resolve_build_args() {
    ...
    case "$target" in
        ...
        xts|xts-dynamic|xts_dynamic) build_args="$BUILD_XTS" ;;
        xts-static|xts_static) build_args="$BUILD_XTS_STATIC" ;;
        *)
...
cmd_build_impl() {
    ...
    if [[ "$target" == "xts-all" || "$target" == "xts_all" || "$target" == "xts-full" || "$target" == "xts_full" ]]; then
        info "=== XTS full: dynamic build ==="
        cmd_build_impl "$fast_rebuild" xts "$@" || return $?
        info "=== XTS full: static build ==="
        cmd_build_impl "$fast_rebuild" xts-static "$@"
        return $?
    fi

    if [[ "$target" == "xts" || "$target" == "xts-dynamic" || "$target" == "xts_dynamic" ]]; then
        cleanup_repo_root_temp_venv
        info "Building: ./test/xts/acts/build.sh product_name=rk3568 system_size=standard xts_suitetype=hap_dynamic $*"
        env -u PYTHONPATH PYTHONNOUSERSITE=1 ./test/xts/acts/build.sh \
            product_name=rk3568 system_size=standard xts_suitetype=hap_dynamic "$@"
        return $?
    fi

    if [[ "$target" == "xts-static" || "$target" == "xts_static" ]]; then
        local hvigor_dir="prebuilts/command-line-tools/hvigor"
        local ci_hvigor_cache="${OHOS_TOOLS_DIR:-}/hvigor-ci"
        local ci_hvigor_url="${OHOS_HVIGOR_CI_URL:-}"
        local ci_hvigor_branch="${OHOS_HVIGOR_CI_BRANCH:-}"
        if [[ ! -f "$hvigor_dir/bin/hvigorw" ]] || ! grep -q "arkts1.2" "$hvigor_dir/hvigor/package.json" 2>/dev/null; then
            rm -rf prebuilts/command-line-tools 2>/dev/null
            if [[ -f "$ci_hvigor_cache/hvigor/bin/hvigorw" ]]; then
                info "Installing CI hvigor from local cache: $ci_hvigor_cache"
                cp -a "$ci_hvigor_cache" prebuilts/command-line-tools
            else
                info "Installing CI hvigor (arkTS 1.2 support) from git..."
                git clone "$ci_hvigor_url" -b "$ci_hvigor_branch" prebuilts/command-line-tools 2>&1 | tail -1
            fi
        fi
        # Ensure ets1.2 SDK directory exists (needed by CI hvigor for static builds)
        local ets_dir="prebuilts/ohos-sdk/linux/26/ets"
        if [[ -d "$ets_dir/static" ]] && [[ ! -d "$ets_dir/ets1.2" ]]; then
            info "Creating ets1.2 -> static symlink in SDK"
            ln -sf static "$ets_dir/ets1.2"
        fi
        # Apply CI preCompile workarounds (PRs the CI merges before build)
        apply_build_workarounds "xts-static" "$(pwd)" || {
            warn "Some xts-static workarounds failed; build may still work"
        }
        # Use CI hvigor (debug2 branch) — fixes schema validation, SDK dir mapping
        swap_hvigor_ci
        cleanup_repo_root_temp_venv
        info "Building: ./test/xts/acts/build.sh product_name=rk3568 system_size=standard xts_suitetype=hap_static $*"
        xts_static_rc=0
        env -u PYTHONPATH PYTHONNOUSERSITE=1 ./test/xts/acts/build.sh \
            product_name=rk3568 system_size=standard xts_suitetype=hap_static "$@" || xts_static_rc=$?
        # Revert workarounds after build (restore pristine state)
        revert_build_workarounds "xts-static" "$(pwd)" 2>/dev/null || true
        return $xts_static_rc
    fi
    ...
    cleanup_repo_root_temp_venv
    info "Building: PYTHONNOUSERSITE=1 ./build.sh ${build_mode_prefix}${build_args} $*"
    env -u PYTHONPATH PYTHONNOUSERSITE=1 ./build.sh "${build_mode_args[@]}" $build_args "$@"
}
...
_parse_xts_args() {
    ...
    local __bundle_name=""
    local __module_name=""
    local __framework_mode="auto"
    local __part_name=""
    local __subsystem_name=""
    local __suite_name=""
    local __case_name=""
    local __productform="${OHOS_DEVELOPER_TEST_PRODUCTFORM:-phone}"
    local __coverage=false
    local __run_all=false
    local __repeat_count=0
    local __device=""
    local __hdc_endpoint="${XTS_HDC_ENDPOINT:-}"
    local __explicit_hdc_path=""
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --all)
                ...
                ;;
            auto|developer_test)
                if [ -z "$__query" ] && [ -z "$__bundle_name" ] && [ "$__framework_mode" = "auto" ]; then
                    __framework_mode="$1"
                    shift
                    continue
                fi
    ...
    case "$__framework_mode" in
        auto|bundle|developer_test) ;;
        *)
            err "test self-test: unsupported --framework value: $__framework_mode"
            exit 1
            ;;
    esac

    if [ "$__run_all" = "true" ] && [ -n "$__bundle_name" ]; then
        err "test self-test: --all cannot be combined with --bundle"
        exit 1
    fi

    XTS_QUERY="$__query"
    XTS_BUNDLE_NAME="$__bundle_name"
    XTS_MODULE_NAME="$__module_name"
    XTS_FRAMEWORK_MODE="$__framework_mode"
    XTS_PART_NAME="$__part_name"
    XTS_SUBSYSTEM_NAME="$__subsystem_name"
    XTS_SUITE_NAME="$__suite_name"
    XTS_CASE_NAME="$__case_name"
    XTS_PRODUCTFORM="$__productform"
    XTS_COVERAGE="$__coverage"
    XTS_RUN_ALL="$__run_all"
    XTS_REPEAT_COUNT="$__repeat_count"
    XTS_DEVICE="$__device"
    XTS_HDC_ENDPOINT="$__hdc_endpoint"
    XTS_EXPLICIT_HDC_PATH="$__explicit_hdc_path"
    XTS_DRY_RUN="$__dry_run"
}

_resolve_xts_test_target() {
    local __summary_json=""
    if [ -n "$XTS_QUERY" ]; then
        __summary_json="$(discover_self_test_component_json "$XTS_QUERY")" || exit $?
    fi
    XTS_SUMMARY_JSON="$__summary_json"

    local __prefers_framework=false
    if [ "$XTS_RUN_ALL" = "true" ] || [ -n "$XTS_SUITE_NAME" ] || [ -n "$XTS_CASE_NAME" ] || [ "$XTS_COVERAGE" = "true" ] || [ "$XTS_REPEAT_COUNT" -gt 0 ] || [ -n "$XTS_PART_NAME" ] || [ -n "$XTS_SUBSYSTEM_NAME" ]; then
        __prefers_framework=true
    fi

    local __bundle_candidate=""
    if [ -z "$XTS_BUNDLE_NAME" ] && [ -n "$__summary_json" ]; then
        __bundle_candidate="$(resolve_self_test_bundle_candidate "$__summary_json" 2>/dev/null || true)"
    fi
    XTS_BUNDLE_CANDIDATE="$__bundle_candidate"

    if [ "$XTS_FRAMEWORK_MODE" = "auto" ]; then
        if [ "$XTS_RUN_ALL" = "true" ]; then
            XTS_FRAMEWORK_MODE="developer_test"
        elif [ -n "$XTS_BUNDLE_NAME" ]; then
            XTS_FRAMEWORK_MODE="bundle"
        elif [ "$__prefers_framework" = "true" ]; then
            XTS_FRAMEWORK_MODE="developer_test"
        elif [ -n "$__bundle_candidate" ]; then
            XTS_FRAMEWORK_MODE="bundle"
        else
            XTS_FRAMEWORK_MODE="developer_test"
        fi
...
_prepare_xts_environment() {
    if [ "$XTS_FRAMEWORK_MODE" = "bundle" ]; then
        if [ "$XTS_RUN_ALL" = "true" ] || [ "$XTS_COVERAGE" = "true" ] || [ "$XTS_REPEAT_COUNT" -gt 0 ] || [ -n "$XTS_SUITE_NAME" ] || [ -n "$XTS_CASE_NAME" ] || [ -n "$XTS_PART_NAME" ] || [ -n "$XTS_SUBSYSTEM_NAME" ]; then
            err "test self-test: --all/--coverage/--repeat/--suite/--case/--part/--subsystem are supported only with --framework developer_test"
            exit 1
        fi
        local __discovery_note=""
        if [ -z "$XTS_BUNDLE_NAME" ]; then
            [ -n "$XTS_QUERY" ] || {
                err "test self-test: provide a component/path or explicit --bundle [--module]"
                exit 1
            }
            if [ -z "$XTS_BUNDLE_CANDIDATE" ]; then
                resolve_self_test_bundle_candidate "$XTS_SUMMARY_JSON" >/dev/null
                exit $?
            fi
            IFS=$'\t' read -r XTS_BUNDLE_NAME XTS_MODULE_NAME __discovery_note <<<"$XTS_BUNDLE_CANDIDATE"
            info "Auto-discovered bundle-backed self-test metadata from ${__discovery_note}"
        fi
    fi
}

_run_xts_test() {
    case "${XTS_FRAMEWORK_MODE:?}" in
        bundle|developer_test) ;;
        *) err "_run_xts_test: invalid XTS_FRAMEWORK_MODE=$XTS_FRAMEWORK_MODE"; return 1 ;;
    esac

    if [ "$XTS_FRAMEWORK_MODE" = "bundle" ]; then
        XTS_MODULE_NAME="${XTS_MODULE_NAME:-entry}"

        local __resolved_hdc_path=""
        if [ -n "$XTS_EXPLICIT_HDC_PATH" ]; then
            __resolved_hdc_path="$XTS_EXPLICIT_HDC_PATH"
        elif __resolved_hdc_path="$(resolve_preferred_hdc_path 2>/dev/null)"; then
            :
        else
            __resolved_hdc_path="$(command -v hdc 2>/dev/null || true)"
        fi

        local __hdc_cmd=()
        if [ -n "$__resolved_hdc_path" ]; then
            __hdc_cmd+=("$__resolved_hdc_path")
        else
            __hdc_cmd+=("hdc")
        fi
        if [ -n "$XTS_HDC_ENDPOINT" ]; then
            __hdc_cmd+=("-s" "$XTS_HDC_ENDPOINT")
        fi
        if [ -n "$XTS_DEVICE" ]; then
            __hdc_cmd+=("-t" "$XTS_DEVICE")
        fi
        __hdc_cmd+=(shell aa test -b "$XTS_BUNDLE_NAME" -m "$XTS_MODULE_NAME" -s unittest OpenHarmonyTestRunner)

        local __env_cmd=()
        __env_cmd+=(env)
        __env_cmd+=("PATH=$(prepend_hdc_bin_dir_to_path "${__resolved_hdc_path:-}")")
        local __hdc_lib_dir=""
        if __hdc_lib_dir="$(detect_hdc_library_path "${__resolved_hdc_path:-}" 2>/dev/null)"; then
            __env_cmd+=("LD_LIBRARY_PATH=${__hdc_lib_dir}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}")
        fi

        if [ "$XTS_DRY_RUN" = "true" ]; then
            info "Self-test command:"
            shell_join_command "${__env_cmd[@]}" "${__hdc_cmd[@]}"
            return 0
        fi

        info "Running developer self-test bundle '${XTS_BUNDLE_NAME}' module '${XTS_MODULE_NAME}'"
        ohos_run_foreground "${__env_cmd[@]}" "${__hdc_cmd[@]}"
        return 0
    fi

    if [ -n "$XTS_BUNDLE_NAME" ]; then
        err "test self-test: --bundle is only valid with bundle mode"
        exit 1
    fi

    local __runner=""
    if ! __runner="$(resolve_developer_test_runner)"; then
        if [ -n "$XTS_SUMMARY_JSON" ] && [ -z "$XTS_BUNDLE_CANDIDATE" ]; then
            err "Component '${XTS_PART_NAME:-${XTS_QUERY:-unknown}}' does not expose bundle-backed self-test metadata for automatic \`aa test\`."
    ...
    if [ "$XTS_RUN_ALL" = "false" ] && [ -z "$XTS_PART_NAME" ] && [ -z "$XTS_QUERY" ]; then
        err "test self-test: provide a component/path, --part, or --all for developer_test mode"
        exit 1
    fi

    if [ -n "$XTS_SUMMARY_JSON" ]; then
        local __framework_candidate=""
        __framework_candidate="$(resolve_self_test_framework_candidate "$XTS_SUMMARY_JSON")"
        local __discovered_part=""
        local __discovered_subsystem=""
        local __discovered_module=""
        IFS=$'\t' read -r __discovered_part __discovered_subsystem __discovered_module <<<"$__framework_candidate"
        XTS_PART_NAME="${XTS_PART_NAME:-$__discovered_part}"
        XTS_SUBSYSTEM_NAME="${XTS_SUBSYSTEM_NAME:-$__discovered_subsystem}"
        XTS_MODULE_NAME="${XTS_MODULE_NAME:-$__discovered_module}"
    fi

    local __framework_run_args=()
    __framework_run_args+=(-p "$XTS_PRODUCTFORM" -t UT)
    if [ -n "$XTS_SUBSYSTEM_NAME" ]; then
        __framework_run_args+=(-ss "$XTS_SUBSYSTEM_NAME")
    fi
    if [ "$XTS_RUN_ALL" != "true" ] && [ -n "$XTS_PART_NAME" ]; then
        __framework_run_args+=(-tp "$XTS_PART_NAME")
    fi
    if [ -n "$XTS_MODULE_NAME" ]; then
        __framework_run_args+=(-tm "$XTS_MODULE_NAME")
    fi
    if [ -n "$XTS_SUITE_NAME" ]; then
        __framework_run_args+=(-ts "$XTS_SUITE_NAME")
    fi
    if [ -n "$XTS_CASE_NAME" ]; then
        __framework_run_args+=(-tc "$XTS_CASE_NAME")
    fi
    if [ "$XTS_COVERAGE" = "true" ]; then
        __framework_run_args+=(-cov)
    fi
    if [ "$XTS_REPEAT_COUNT" -gt 0 ]; then
        __framework_run_args+=(--repeat "$XTS_REPEAT_COUNT")
    fi

    local __summary_xml=""
    __summary_xml="$(developer_test_latest_summary_xml "$__runner" 2>/dev/null || true)"

    info "Running developer self-test via developer_test framework"
    local __use_tdd_runner=false
    if [ -n "$XTS_DEVICE" ] || [ -n "$XTS_EXPLICIT_HDC_PATH" ] || [ -n "$XTS_HDC_ENDPOINT" ]; then
        __use_tdd_runner=true
    fi
    if [ "$__use_tdd_runner" = "true" ]; then
        local __resolved_hdc_path=""
        if [ -n "$XTS_EXPLICIT_HDC_PATH" ]; then
            __resolved_hdc_path="$XTS_EXPLICIT_HDC_PATH"
        elif __resolved_hdc_path="$(resolve_preferred_hdc_path 2>/dev/null)"; then
        ...
        local __tdd_runner_args=()
        __tdd_runner_args+=(--repo-root "$(pwd)")
        __tdd_runner_args+=(--runner-path "$__runner")
        if [ -n "$__resolved_hdc_path" ]; then
            __tdd_runner_args+=(--hdc-path "$__resolved_hdc_path")
        fi
        if [ -n "$XTS_HDC_ENDPOINT" ]; then
            __tdd_runner_args+=(--hdc-endpoint "$XTS_HDC_ENDPOINT")
        fi
        if [ -n "$XTS_DEVICE" ]; then
            __tdd_runner_args+=(--device "$XTS_DEVICE")
        fi
        if [ "$XTS_DRY_RUN" = "true" ]; then
            __tdd_runner_args+=(--dry-run)
        fi
        __tdd_runner_args+=(--framework-args "${__framework_run_args[@]}")
        run_tdd_runner_tool "${__tdd_runner_args[@]}"
        return $?
    fi

    local __framework_cmd=()
    __framework_cmd+=("$__runner" run "${__framework_run_args[@]}")
    if [ "$XTS_DRY_RUN" = "true" ]; then
        info "Self-test command:"
        shell_join_command "${__framework_cmd[@]}"
        if [ -n "$__summary_xml" ]; then
            info "Expected summary XML: ${__summary_xml}"
        fi
        return 0
    fi
    ohos_run_foreground "${__framework_cmd[@]}"
    if [ -n "$__summary_xml" ] && [ -f "$__summary_xml" ]; then
        print_developer_test_summary "$__summary_xml" || true
    else
        warn "developer_test finished, but no summary_report.xml was found at: ${__summary_xml:-<unknown>}"
    fi
}
...
cmd_test_built_run() {
    ...
    local component=""
    local build_target=""
    local product="rk3568"
    local ssh_host=""
    local ssh_user="${OHOS_SSH_USER:-$USER}"
    local hdc_path="${HDC_PATH:-hdc}"
    local hdc_endpoint=""
    local filter=""
    local dry_run=false
    ...
    while [ $# -gt 0 ]; do
        case "$1" in
            ...
            --build-target=*) build_target="${1#*=}"; shift ;;
            ...
            --hdc-path) hdc_path="$2"; shift 2 ;;
            --hdc-path=*) hdc_path="${1#*=}"; shift ;;
            --hdc-endpoint) hdc_endpoint="$2"; shift 2 ;;
            --hdc-endpoint=*) hdc_endpoint="${1#*=}"; shift ;;
            ...
            *)
                if [ -z "$component" ]; then
                    ...
                    err "built-run: unknown argument: $1"
                    return 1
                fi
                ;;
        esac
    ...
    [ -n "$component" ] || { err "built-run: component is required (e.g. ace_engine)"; return 1; }
    [ -n "$build_target" ] || build_target="${component}_test"
    if [ -n "$ssh_host" ]; then
        local mode="ssh_relay"
    elif [ -n "$hdc_endpoint" ]; then
        local mode="direct_hdc"
    else
        ssh_host="${OHOS_DEVICE_SERVER_HOST:-}"
        if [ -n "$ssh_host" ]; then
            local mode="ssh_relay"
        else
            err "built-run: provide --ssh-host (or set OHOS_DEVICE_SERVER_HOST) for SSH relay, or --hdc-endpoint for direct HDC mode"
            return 1
        fi
    fi
    ...
    if [ "$mode" = "ssh_relay" ]; then
        info "Mode: SSH relay via ${ssh_user}@${ssh_host}"
    else
        info "Mode: direct HDC via $hdc_endpoint"
    fi
    info "HDC path: $hdc_path"

    # Pre-flight: check connectivity
    if [ "$mode" = "direct_hdc" ]; then
        info "Checking HDC connectivity to $hdc_endpoint ..."
        if ! $hdc_path -s "$hdc_endpoint" list targets 2>/dev/null | grep -q '[^[:space:]]'; then
            err "HDC endpoint not reachable: $hdc_endpoint"
            echo ""
            echo "  Troubleshooting:"
            echo "    On the Windows client with the board connected via USB:"
            echo "      1. hdc kill"
            echo "      2. hdc start -r -p 8710"
            echo "      3. hdc list targets   # should list device"
            echo "    On this server:"
            echo "      4. nc -zv ${hdc_endpoint%:*} ${hdc_endpoint#*:}   # check TCP"
            echo "    If the relay binds to 127.0.0.1 only, use SSH tunnel:"
            echo "      ssh -L 8710:localhost:8710 user@${hdc_endpoint%:*}"
            return 1
        fi
        local targets
        targets="$($hdc_path -s "$hdc_endpoint" list targets 2>/dev/null | tr '\n' ' ')"
        info "  Device(s) found: ${targets:-<none>}"
    elif [ "$mode" = "ssh_relay" ]; then
        ...
        if ! ssh -o BatchMode=yes -o ConnectTimeout=5 "${ssh_user}@${ssh_host}" true 2>/dev/null; then
            err "SSH host not reachable: ${ssh_user}@${ssh_host}"
            echo ""
            echo "  Troubleshooting:"
            echo "    1. ping $ssh_host"
            echo "    2. ssh ${ssh_user}@${ssh_host}"
            echo "    3. Check that HDC is in PATH on the relay host"
            return 1
        fi
    ...
    if [ "$dry_run" = "true" ]; then
        info "[dry-run] Step 1: build"
        info "[dry-run]   ./build.sh --product-name \"$product\" --build-target \"$build_target\" --ccache"
        info "[dry-run] Step 2: discover binaries in out/$product/tests/unittest/$component/"
        info "[dry-run] Step 3: create tar archive of all binaries, push once, extract on device"
        info "[dry-run] Step 4: run each binary from extracted directory, collect results"
        info "[dry-run] Step 5: clean up archive and extracted files from device"
        info "[dry-run]   (re-run without --dry-run to execute all steps)"
        info "[dry-run]   (add --skip-build to skip the build step)"
        return 0
    fi
    if [ "$skip_build" = "true" ]; then
        info "Skipping build (--skip-build). Using existing binaries in out/$product/tests/unittest/$component/"
    else
        info "Building $build_target ..."
        if ! ohos_run_foreground ./build.sh --product-name "$product" --build-target "$build_target" --ccache; then
            err "build failed for target $build_target"
            return 1
        fi
    fi

    # Step 2: find test binaries
    local test_dir="out/$product/tests/unittest/$component"
    if [ ! -d "$test_dir" ]; then
    ...
    while IFS= read -r -d '' f; do
        [ -f "$f" ] && [ -x "$f" ] && binaries+=("$f")
    done < <(find "$test_dir" -type f -executable -print0 2>/dev/null || true)

    if [ ${#binaries[@]} -eq 0 ]; then
        err "no test binaries found in $test_dir"
        return 1
    fi
    ...
    info "Found ${#binaries[@]} test binary(ies), total size: $(numfmt --to=iec $total_size 2>/dev/null || echo "${total_size}B")"
    if [ "$total_size" -gt $((500 * 1024 * 1024)) ]; then
        local transfer_estimate=$(( total_size / (10 * 1024 * 1024) ))
        warn "Large transfer (~$(numfmt --to=iec $total_size 2>/dev/null || echo "${total_size}B")). Estimated ~${transfer_estimate}s at 10 MB/s."
    fi
    for b in "${binaries[@]}"; do
        info "  $(basename "$b")"
    done
    ...
    if [ -n "$filter" ]; then
        local filtered=()
        for b in "${binaries[@]}"; do
            if [[ "$(basename "$b")" == *"$filter"* ]]; then
                filtered+=("$b")
            fi
        done
        if [ ${#filtered[@]} -eq 0 ]; then
            err "filter '$filter' matched no binaries"
            return 1
        fi
        binaries=("${filtered[@]}")
        info "Filtered to ${#binaries[@]} binary(ies) matching '$filter'"
    fi

    # Step 3: create archive and push to device
    local hdc_cmd="$hdc_path"
    if [ -n "$hdc_endpoint" ]; then
        hdc_cmd="$hdc_path -s $hdc_endpoint"
    fi

    local archive_dir
    archive_dir="$(mktemp -d /tmp/ohos_test_archive_XXXXXX)"
    local archive_name="${component}_tests.tar.gz"
    local archive_path="$archive_dir/$archive_name"
    local remote_root="/data/local/tmp"
    local remote_dir="$remote_root/ohos_${component}_tests"

    local b
    for b in "${binaries[@]}"; do
        cp "$b" "$archive_dir/" 2>/dev/null || true
    done
    tar czf "$archive_path" -C "$archive_dir" . 2>/dev/null
    local archive_size
    archive_size="$(stat -c%s "$archive_path" 2>/dev/null || echo 0)"
    info "Archive: $(numfmt --to=iec $archive_size 2>/dev/null || echo "${archive_size}B") for ${#binaries[@]} binaries"

    if [ "$mode" = "ssh_relay" ]; then
        local remote_archive="/tmp/$archive_name"
        if ! scp "$archive_path" "${ssh_user}@${ssh_host}:$remote_archive" >/dev/null 2>&1; then
            err "scp failed for test archive"
            rm -rf "$archive_dir" "$archive_path"
            return 1
        fi
        {
            printf 'set -e\n'
            printf '%s shell "mkdir -p %s"\n' "$hdc_cmd" "$remote_dir"
            printf '%s file send %s %s\n' "$hdc_cmd" "$remote_archive" "$remote_root/"
            printf '%s shell "cd %s && tar xzf %s/%s"\n' "$hdc_cmd" "$remote_dir" "$remote_root" "$archive_name"
            printf '%s shell chmod +x %s/*\n' "$hdc_cmd" "$remote_dir"
            printf 'rm -f %s\n' "$remote_archive"
        } | ssh -o BatchMode=yes -o ConnectTimeout=5 "${ssh_user}@${ssh_host}" bash -l >/dev/null 2>&1
        local push_rc=${PIPESTATUS[1]}
    else
        $hdc_cmd shell "mkdir -p $remote_dir" 2>/dev/null
        if ! $hdc_cmd file send "$archive_path" "$remote_root/" 2>/dev/null; then
            err "HDC file send failed for test archive"
            rm -rf "$archive_dir" "$archive_path"
            return 1
        fi
        $hdc_cmd shell "cd $remote_dir && tar xzf $remote_root/$archive_name" 2>/dev/null
        $hdc_cmd shell "chmod +x $remote_dir/*" 2>/dev/null
        local push_rc=0
    fi

    if [ "$push_rc" -ne 0 ]; then
        err "Failed to push test archive to device"
        rm -rf "$archive_dir" "$archive_path"
        return 1
    fi
    info "Archive pushed and extracted on device"

    rm -rf "$archive_dir" "$archive_path"

    # Step 4: run each test binary from the extracted directory
    local results_dir=".runtime/test_results/$component"
    mkdir -p "$results_dir"
    ...
    for b in "${binaries[@]}"; do
        ...
        if [ "$mode" = "ssh_relay" ]; then
            local tmp_log
            tmp_log="$(mktemp /tmp/ohos_test_run_XXXXXX.log)"
            printf '%s shell "cd %s && ./%s"\n' "$hdc_cmd" "$remote_dir" "$name" \
                | ssh -o BatchMode=yes -o ConnectTimeout=5 "${ssh_user}@${ssh_host}" bash -l > "$tmp_log" 2>&1
            rc=${PIPESTATUS[1]}
            mv "$tmp_log" "$logfile" 2>/dev/null || true
        else
            local tmp_log
            tmp_log="$(mktemp /tmp/ohos_test_run_XXXXXX.log)"
            $hdc_cmd shell "cd $remote_dir && ./$name" > "$tmp_log" 2>&1
            rc=$?
            mv "$tmp_log" "$logfile" 2>/dev/null || true
        fi
        ...
        awk 'NF' "$logfile" 2>/dev/null | tail -5 | sed 's/^/    /'
    done

    # Step 5: clean up device
    if [ "$mode" = "ssh_relay" ]; then
        printf '%s shell "rm -rf %s %s/%s"\n' "$hdc_cmd" "$remote_dir" "$remote_root" "$archive_name" \
            | ssh -o BatchMode=yes -o ConnectTimeout=5 "${ssh_user}@${ssh_host}" bash -l >/dev/null 2>&1 || true
    else
        $hdc_cmd shell "rm -rf $remote_dir $remote_root/$archive_name" 2>/dev/null || true
    fi
    ...
    # Failed binaries and hints
    if [ "$failed" -gt 0 ]; then
        local fatal_count=0 segfault_count=0 missing_dep_count=0 empty_log_count=0
        local f
        for f in "${failed_names[@]}"; do
            local logfile="$results_dir/${f}.log"
            [ -f "$logfile" ] || continue
            grep -q 'FATAL' "$logfile" 2>/dev/null && ((++fatal_count))
            grep -qi 'segfault\|signal\|SIGABRT\|SIGSEGV\|SIGBUS' "$logfile" 2>/dev/null && ((++segfault_count))
            grep -qi 'No such file or directory' "$logfile" 2>/dev/null && ((++missing_dep_count))
            [ ! -s "$logfile" ] && ((++empty_log_count))
        done

        echo ""
        err "Failed binaries (${#failed_names[@]}):"
        for f in "${failed_names[@]}"; do
            warn "    $f (log: $results_dir/$f.log)"
        done

        echo ""
        info "Failure hints:"
        [ "$fatal_count" -gt 0 ] && info "  - $fatal_count binary(ies) had gtest FATAL (XML write error)" && info "    Fixed by running inside $remote_dir (writable CWD)"
        [ "$segfault_count" -gt 0 ] && info "  - $segfault_count binary(ies) crashed (signal/segfault)" && info "    Check hardware support or run manually"
        [ "$missing_dep_count" -gt 0 ] && info "  - $missing_dep_count binary(ies) missing shared libs" && info "    Build statically or install missing libs on device"
        [ "$empty_log_count" -gt 0 ] && info "  - $empty_log_count binary(ies) produced no output" && info "    Binary may have failed before writing anything"
        if [ "$fatal_count" -eq 0 ] && [ "$segfault_count" -eq 0 ] && [ "$missing_dep_count" -eq 0 ] && [ "$empty_log_count" -eq 0 ]; then
            info "  - Check individual logs: less $results_dir/<name>.log"
        fi
        info "  - Re-run a single binary:"
        [ "$mode" = "direct_hdc" ] && info "      $hdc_cmd shell \"cd $remote_dir && ./<name> 2>&1\""
        [ "$mode" = "ssh_relay" ] && info "      ssh ${ssh_user}@${ssh_host} \"$hdc_cmd shell cd $remote_dir && ./<name>\""
    fi
}
...
cmd_pr() {
    ...
    case "$subcmd" in
        ...
        show-comments)
            ...
            if env ${GITEE_UTIL_PYTHONPATH:+"PYTHONPATH=$GITEE_UTIL_PYTHONPATH"} "$GITEE_UTIL_PYTHON" "$GITEE_UTIL_RUNNER" "${provider_args[@]}" "$subcmd" "$@" >"$comments_output" 2>&1; then
            ...
            return "$comments_rc"
            ;;
        create-pr|create-issue|create-issue-pr|comment-pr|list-pr|show-pr|apply-pr)
            ensure_gitee_util_runtime
            ohos_run_foreground env ${GITEE_UTIL_PYTHONPATH:+"PYTHONPATH=$GITEE_UTIL_PYTHONPATH"} "$GITEE_UTIL_PYTHON" "$GITEE_UTIL_RUNNER" "${provider_args[@]}" "$subcmd" "$@"
            ;;
        ci-commands)
            # Delegates to gitee_query.py — read-only, no auth needed
            ensure_gitee_util_runtime
            local _gq="${SCRIPT_DIR}/gitee_util/gitee_query.py"
            if [ ! -f "$_gq" ]; then
                err "Missing: $_gq"
                exit 1
            fi
            ohos_run_foreground env ${GITEE_UTIL_PYTHONPATH:+"PYTHONPATH=$GITEE_UTIL_PYTHONPATH"} \
                "${GITEE_UTIL_PYTHON:-python3}" "$_gq" "${provider_args[@]}" list-ci-commands "$@"
            ;;
        ci-command)
            # Posts a CI bot command as a PR comment — needs auth
            ensure_gitee_util_runtime
            local _gq="${SCRIPT_DIR}/gitee_util/gitee_query.py"
            if [ ! -f "$_gq" ]; then
                err "Missing: $_gq"
            ...
            ohos_run_foreground env ${GITEE_UTIL_PYTHONPATH:+"PYTHONPATH=$GITEE_UTIL_PYTHONPATH"} \
                "${GITEE_UTIL_PYTHON:-python3}" "$_gq" "${provider_args[@]}" ci-command "$@"
            ;;
        *)
...
run_pack_tool() {
    if [ ! -f "$OHOS_PACK_TOOL" ]; then
        err "Missing pack tool: $OHOS_PACK_TOOL"
        exit 1
    fi
    exec python3 "$OHOS_PACK_TOOL" "$@"
}

run_sign_tool() {
    if [ ! -f "$OHOS_SIGN_TOOL" ]; then
        err "Missing sign tool: $OHOS_SIGN_TOOL"
        exit 1
    fi
    exec bash "$OHOS_SIGN_TOOL" "$@"
}
...
cmd_status() {
    local status_script="${SCRIPT_DIR}/ohos_status.py"
    if [ ! -f "$status_script" ]; then
        err "status: ${status_script} not found"
        return 1
    fi
...
cmd_admin_relocate() {
    ...
    if [ -z "$target_root" ]; then
        ...
        err "Example: ohos admin relocate --target-root ~/projects/ohos-helper --dry-run"
        exit 1
    fi
...
Tool commands:
  ...
  ci [args]                CI build info and artifact download from PR comments
  download [subcommand]    Download daily SDK / firmware / XTS test packages
  device [subcommand]      Device helpers: init-board, bridge, flash, power, xts-run
  feedback                 Save project feedback / wishes next to this script
  pack <type> [options]    Package build artifacts into stamped archive
  pr [subcommand]          Wrapper around vendored gitee/gitcode PR helper
  sign [subcommand]        Sign HAP packages: profile generation, app signing, verification
  ...
  status [options]         Dashboard: running XTS runs, board usage, flash/build ops
  build-workarounds [subcommand]  Apply/revert PR patches for build targets (CI parity)
...
Useful examples:
  ...
  ohos pack                               Auto-detect + interactive selection menu
  ohos pack firmware                       Package firmware images after build
  ohos pack xts --output /tmp/share        Package XTS suites to custom location
  ohos pack --dry-run                       Preview all packable artifacts
  ...
  ohos ci openharmony arkui_ace_engine 84616
  ohos ci openharmony arkui_ace_engine 84616 download
...
Preflight checks:
  ...
  - ensure sibling link exists:
      <repo-parent>/${PREBUILTS_SYMLINK_NAME:-PREBUILTS_SYMLINK_NAME} -> ${SHARED_PREBUILTS_DIR:-SHARED_PREBUILTS_DIR}

Common aliases:
  sdk                 ./build.sh ${BUILD_SDK:-<build_args>}
  sdk-linux           ./build.sh ${BUILD_SDK_LINUX:-<build_args>}
  sdk-win             ./build.sh ${BUILD_SDK_WIN:-<build_args>}
  rk3568              ./build.sh ${BUILD_RK3568:-<build_args>}
  xts                 ./test/xts/acts/build.sh ... xts_suitetype=hap_dynamic
  xts-dynamic         same as xts (explicit alias)
  xts-static          ./test/xts/acts/build.sh ... xts_suitetype=hap_static
  xts-all             xts + xts-static in sequence
...
Usage:
  ...
  ohos test built-run <component> [--build-target TARGET] [--product PRODUCT] [--ssh-host HOST] [--ssh-user USER] [--hdc-path PATH] [--hdc-endpoint HOST:PORT] [--filter PATTERN] [--skip-build] [--dry-run]
...
Subcommands:
  ...
  built-run
      Build <component> device-test targets (ohos_unittest), push them to a
      remote board via HDC, run them, and print a summary with failure hints.
      Flow:
        0. Pre-flight: check HDC/SSH connectivity, show troubleshooting tips
        1. ./build.sh --product-name <product> --build-target <target> --ccache (skip with --skip-build)
        2. scan out/<product>/tests/unittest/<component>/ for test binaries
        3. create tar archive of all binaries -> push once -> extract on device
        4. run each binary from extracted dir -> log -> summary with failure hints
        5. cleanup archive + extracted files from device
      Defaults:
        --build-target  <component>_test
        --product       rk3568
        --ssh-user      $OHOS_SSH_USER or $USER
        --ssh-host      uses OHOS_DEVICE_SERVER_HOST if set
        --hdc-path      $HDC_PATH or auto-detect
      Options:
        --filter PATTERN  only run binaries whose name contains PATTERN
        --skip-build      skip the build step (use existing binaries)
      Modes:
        SSH relay (default):
            Provide --ssh-host (or set OHOS_DEVICE_SERVER_HOST).
            The relay host needs: scp + ssh server + hdc in PATH + board via USB.
        Direct HDC (Windows client):
            Provide --hdc-endpoint <win-ip>:8710 (no --ssh-host).
            On the Windows client run:
              hdc start -r -p 8710
            Then the board is reachable from this server via HDC TCP relay.
      Notes:
        - Pre-flight checks verify connectivity before building
        - Binary inventory shows count + total size before transfer
        - On failure, script shows which binaries failed and why
        - Summary includes troubleshooting hints per failure type
        - works with standalone ohos_unittest targets (gtest-style)
        - does NOT use developer_test framework
...
Examples:
  ...
  ohos test built-run ace_engine --ssh-host 10.0.0.10 --dry-run
  ohos test built-run ace_engine --ssh-host 10.0.0.10 --build-target ace_engine_test
  ohos test built-run ace_engine --hdc-endpoint 10.0.0.10:8710 --dry-run
HELP
}
...
Examples:
  ...
  ohos file ./foundation/arkui/ace_engine/test/unittest/capi/modifiers/form_link_modifier_test.cpp
  ohos file /path/to/ohos_master/foundation/arkui/ace_engine/test/unittest/capi/modifiers/form_link_modifier_test.cpp
...
Supported subcommands:
  ...
  show-comments
  apply-pr
  ci-commands         — list CI bot commands (start build, lgtm, check dco, ...)
  ci-command          — post a CI bot command as a comment to a PR
...
Notes:
  ...
  - ci-commands reads from gitee_util/pr_commands.json (parsed from official
    OpenHarmony docs: openharmony/community build_command.md).
...
Examples:
  ...
  ohos pr apply-pr https://gitcode.com/openharmony/developtools_ace_ets2bundle/pulls/6737
  ohos pr apply-pr --strategy checkout https://gitcode.com/openharmony/arkui_ace_engine/pulls/12345
  ohos pr ci-commands                                # list all CI bot commands
  ohos pr ci-commands --labels                       # show PR state labels
  ohos pr ci-commands --json                         # raw JSON output
  ohos pr ci-command start build --url https://gitcode.com/owner/repo/pulls/123
  ohos pr ci-command lgtm --url https://gitcode.com/owner/repo/pulls/456
  ohos pr ci-command "assign @user1 @user2" --url https://gitcode.com/owner/repo/pulls/789
HELP
}
...
print_help_pack() {
    if [ -f "$OHOS_PACK_TOOL" ]; then
        python3 "$OHOS_PACK_TOOL" --help
        return
    fi
    err "Missing pack tool: $OHOS_PACK_TOOL"
}

OHOS_CI_TOOL="${OHOS_CI_TOOL:-${SCRIPT_DIR}/ohos_ci_tool.py}"

print_help_ci() {
    if [ -f "$OHOS_CI_TOOL" ]; then
        python3 "$OHOS_CI_TOOL" --help
        return
    fi
    err "Missing CI tool: $OHOS_CI_TOOL"
}
...
cmd_help() {
    ...
    case "$topic" in
        ...
        pack) print_help_pack ;;
        sign) bash "$OHOS_SIGN_TOOL" help ;;
        ci) print_help_ci ;;
...
dispatch_single_command() {
    ...
    case "$cmd" in
        ...
        ci) cmd_ci "$@" ;;
        status) cmd_status "$@" ;;
        build-workarounds) cmd_build_workarounds "$@" ;;
        *)
...
case "$1" in
    help|--help|-h|products|parts|info|file|device|admin|feedback|params|npmrc|config|manifest-save|pr|run|test|xts|download|pack|sign|ci|status|build-workarounds)
        cmd="$1"
        shift
